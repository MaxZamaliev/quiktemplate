...
--[[
	...
	function option.getFullListByBase(baseSecCode) - получить список seccode опционов с базовым инструментом baseSecCode
	function option.getActiveListByBase(baseSecCode) - получить список ликвидных (есть предложания) в данный момент опционов
	...
	function option.countImpliedVolatility(optionStrike,optionsDaysLeft,optionPrice,futuresPrice) -- вычислить волатильность
	function option.Volatility(seccode)   - получить волатильность
	function option.getTheorPrice(seccode)   - получить теоретическую цену
	function option.countTheorPrice(optionStrike,optionDaysLeft,futuresPrice,volatility) -- вычислить теоретеческую стоимость опциона
...
---------------------------------------------------------------------------------------------------------------------------------------------------------
	function option.checkStatus(seccode)
		...
		if image == nil then
			log.Error("option.checkStatus("..tostring(seccode).."): Опцион не найден")
			return nil
		end
		if image.param_image == "" then
			log.Error("option.checkStatus("..seccode.."): Нет данных об опционе")
			return nil
		end

		if image.param_image ~= "торгуется" then
